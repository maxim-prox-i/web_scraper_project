...
class SiteScraper:
    def __init__(self, url, output_file=None, max_urls=None):
        ...
        self.to_visit = [self.start_url]  # URLs à visiter (liste simple au lieu d'une queue)
    ...
    def extract_sitemap_urls(self):
        ...
        try:
            ...
            if sitemaps:
                    ...
                    if loc is not None and loc.text:
                        ...
                        if sub_response and sub_response.status_code == 200:
                            try:
                                ...
                                for url_elem in sub_root.findall('.//' + namespace + 'url//' + namespace + 'loc') or sub_root.findall('.//url//loc'):
                                    if url_elem.text:
                                ...
                                self.logger.info(f"Extrait {len(sitemap_urls)} URLs du sous-sitemap {sub_sitemap_url}")
                                
                            except ET.ParseError as e:
                                self.logger.error(f"Erreur de parsing du sous-sitemap {sub_sitemap_url}: {e}")
            else:
                # C'est un sitemap standard
                for url_elem in root.findall('.//' + namespace + 'url//' + namespace + 'loc') or root.findall('.//url//loc'):
                    if url_elem.text:
                        url = url_elem.text.strip()
                        normalized_url = self.normalize_url(url)
                        if self.is_valid_url(normalized_url):
                ...
                self.logger.info(f"Extrait {len(sitemap_urls)} URLs du sitemap {sitemap_url}")
    ...
    def scrape(self, progress_callback=None):
        ...
        for url in sitemap_urls:
            ...
            if normalized_url not in self.found_urls:
                if progress_callback:
                    progress_callback(len(self.found_urls), self.max_urls or len(sitemap_urls), 
                                    f"Enregistrement de l'URL du sitemap: {normalized_url[:50]}...")
                
                if self.save_url(normalized_url):
                    self.logger.info("Limite d'URLs atteinte après traitement du sitemap.")
                    return self.found_urls
                
                # Ajouter à la liste à visiter si pas déjà visitée
                if normalized_url not in self.visited_urls and normalized_url not in self.to_visit:
        ...
        while self.to_visit:
            ...
            if self.max_urls and len(self.found_urls) >= self.max_urls:
                self.logger.info(f"Limite d'URLs atteinte: {len(self.found_urls)}/{self.max_urls}")
                break
            ...
            if len(self.visited_urls) % 10 == 0:
                self.logger.info(f"Progression: {len(self.found_urls)} URLs trouvées, {len(self.visited_urls)} visitées, {len(self.to_visit)} à visiter")
            
            if progress_callback:
                progress_callback(len(self.found_urls), self.max_urls or 0, 
