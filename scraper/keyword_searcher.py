...
class KeywordSearcher:
    def __init__(
        self, input_file, keywords, case_sensitive=False, temp_dir=None, max_threads=10
    ):
        """
        ...
        self.output_file = os.path.join(
            self.temp_dir, f"keyword_results_{timestamp}.csv"
        )
    ...
    def search_keywords_in_text(self, text, url):
        ...
        content_preview = (
            page_content[:200] + "..." if len(page_content) > 200 else page_content
        )
                ...
                for match in matches_to_process:
                    ...
                    if self.case_sensitive:
                        ...
                        highlighted = context.replace(
                            original_match, f"**{original_match}**"
                        )
    ...
    def run(self, progress_callback=None):
        ...
        self.logger.info(
            f"Recherche de mots-clés dans {total_urls} URLs depuis {self.input_file}"
        )
        self.logger.info(f"Mots-clés: {', '.join(self.keywords)}")

        if progress_callback:
            progress_callback(
                0, total_urls, f"Démarrage de la recherche pour {total_urls} URLs"
            )

        # Initialiser la barre de progression
        self.pbar = tqdm(
            total=total_urls,
            desc="Recherche de mots-clés",
        ...
        with ThreadPoolExecutor(max_workers=self.max_threads) as executor:
            ...
            while processed < total_urls:
                ...
                success_rate = (
                    f"{(urls_with_matches / processed * 100):.1f}%"
                    if processed > 0
                    else "0.0%"
                )
                ...
                self.pbar.set_description(
                    f"Recherche - ETA: {eta_formatted} - Correspondances: {matches_found}"
                )
        ...
        final_status = (
            f"Recherche terminée en {self.format_time(elapsed_time)}\n"
...
if __name__ == "__main__":
    ...
    print(
        "Ce script parcourt une liste d'URLs et recherche des mots-clés spécifiques dans les pages web."
    )
    ...
    case_sensitive = (
        input("Respecter la casse? (o/n, par défaut: non): ").lower() == "o"
    )
