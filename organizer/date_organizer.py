...
class URLDateOrganizer:
    def __init__(self, input_file, output_dir=None, folder_name=None):
        ...
        self.folder_name = (
            self.sanitize_filename(folder_name) if folder_name else "urls_organisees"
        )
    ...
    def organize_urls(self, progress_callback=None):
        ...
        urls_by_date = {}
        total_rows = (
            sum(1 for _ in open(self.input_file, "r", encoding="utf-8")) - 1
        )  # -1 pour l'en-tête
            ...
            for row in reader:
                ...
                if date:
                    ...
                    if match:
                        ...
                        self.stats["months"][month] = (
                            self.stats["months"].get(month, 0) + 1
                        )
        ...
        total_entries = len(self.undated_urls) + sum(
            len(urls) for urls in urls_by_date.values()
        )

        # Initialiser la barre de progression pour l'écriture
        pbar = tqdm(
            total=total_entries,
            desc="Organisation des URLs",
            bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]",
        )
        ...
        if self.undated_urls:
            undated_file = os.path.join(
                self.output_dir, "Sans_Date", "urls_sans_date.txt"
            )
        ...
        # Message final
        final_status = (
            f"Organisation terminée en {self.format_time(elapsed_time)}\n"
        ...
        if self.stats["urls_with_dates"] > 0:
            ...
            if progress_callback:
                progress_callback(
                    total_entries, total_entries, final_status + "\n\n" + year_summary
                )
...
if __name__ == "__main__":
    ...
    print(
        "Ce script organise les URLs en dossiers par année et par mois selon leurs dates de publication."
    )

    input_file = (
        input("Fichier CSV des dates (par défaut: urls-dates.csv): ")
        or "urls-dates.csv"
    )
    ...
    if folder_name:
        ...
        if sanitized_name != folder_name:
            print(
                f"Note: Les caractères spéciaux dans le nom ont été remplacés. Nom utilisé: {sanitized_name}"
            )
            folder_name = sanitized_name

    output_dir = (
        input(
            "Chemin complet du répertoire parent (laissez vide pour utiliser le répertoire actuel): "
        )
        or None
    )
