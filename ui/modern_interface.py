...
class CustomNotebook(ttk.Notebook):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        style = ttk.Style()
        style.configure(
            "CustomNotebook", background=COLORS["background"], borderwidth=0
        )
        style.configure(
            "CustomNotebook.Tab",
...
class ScrollableFrame(ttk.Frame):
    ...
    def __init__(self, container, *args, **kwargs):
        ...
        self.canvas = tk.Canvas(
            self, borderwidth=0, background=COLORS["card"], highlightthickness=0
        )
...
class ModernButton(ttk.Button):
    ...
    def __init__(
        self, parent, text, command=None, icon=None, style="primary", **kwargs
    ):
        """
        ...
        super().__init__(
            parent, text=text, command=command, style="Modern.TButton", **kwargs
        )
...
class ModernWebScraperApp:
    ...
    def create_layout(self):
        ...
        subtitle_label = ttk.Label(
            header_frame,
            text="Extraire, organiser et exporter des données web",
            font=subtitle_font,
    ...
    def create_field(
        self, parent, label_text, variable, browse_command=None, width=40, tooltip=None
    ):
        """
        ...
        # Libellé
        label = ttk.Label(
            field_frame, text=label_text, style="Normal.TLabel", width=25, anchor="w"
        )
        ...
        if browse_command:
            browse_button = ModernButton(
                entry_frame, text="Parcourir", command=browse_command, style="secondary"
            )
    ...
    def _init_keyword_search_tab(self):
        ...
        keywords_entry = ttk.Entry(
            keywords_frame, textvariable=self.keyword_search_terms
        )
        keywords_entry.pack(side="left", fill="x", expand=True)
        ToolTip(
            keywords_entry,
            'Entrez un ou plusieurs mots-clés séparés par des virgules. Utilisez des guillemets pour les expressions exactes, ex: "maison bleue"',
        )
        ...
        case_check = ttk.Checkbutton(
            case_frame, variable=self.keywords_case_sensitive, style="TCheckbutton"
        )
        case_check.pack(side="left")
        ToolTip(
            case_check, "Si coché, la recherche respectera les majuscules/minuscules"
        )
    ...
    def _start_keyword_search(self):
        ...
        if not search_terms:
            messagebox.showerror(
                "Erreur", "Veuillez entrer au moins un mot-clé à rechercher."
            )
            return
        ...
        self.add_log_info(
            self.keyword_log, f"Mots-clés recherchés: {', '.join(keywords)}"
        )
        self.add_log_info(
            self.keyword_log,
            f"Respect de la casse: {'Oui' if case_sensitive else 'Non'}",
        )
        ...
        self.status_bar.set_status(
            f"Recherche de mots-clés en cours pour {input_file}..."
        )
    ...
    def _run_keyword_search(self, input_file, keywords, case_sensitive, max_threads):
        ...
        try:
            ...
            searcher = KeywordSearcher(
                input_file, keywords, case_sensitive, max_threads=max_threads
            )

            # Callback pour mettre à jour la progression
            def update_progress(current, max_val, message):
                eta, elapsed = self._calculate_eta(
                    current, max_val, self.keyword_start_time
                )
                eta_formatted = (
            ...
            if result:
                self.queue.put(
                    ("keyword_complete", result["output_file"], result["stats"])
                )
            else:
                self.queue.put(
                    (
                        "keyword_error",
                        "La recherche n'a pas pu être terminée correctement.",
                    )
                )
    ...
    def _update_keyword_progress(self, current, max_val, status):
        ...
        if max_val > 0:
            self.status_bar.set_status(
                f"Recherche en cours: {current}/{max_val} URLs traitées"
            )
        else:
    ...
    def _keyword_complete(self, output_file, stats):
        ...
        self.add_log_info(
            self.keyword_log, f"URLs avec correspondances: {urls_with_matches}"
        )
        self.add_log_info(
            self.keyword_log, f"Total des correspondances: {total_matches}"
        )
        ...
        if matches_per_keyword:
            ...
            for keyword, count in matches_per_keyword.items():
                self.add_log_info(
                    self.keyword_log, f"  '{keyword}': {count} occurrences"
                )

        self.add_log_info(
            self.keyword_log, f"Les résultats ont été enregistrés dans {output_file}"
        )
        ...
        # Mettre à jour la barre de statut
        self.status_bar.set_status(
            f"Recherche terminée. {total_matches} correspondances trouvées dans {urls_with_matches} URLs."
        )
    ...
    def _keyword_error(self, error):
        ...
        messagebox.showerror(
            "Erreur de recherche", f"Une erreur est survenue:\n{error}"
        )
    ...
    def _init_date_extractor_tab(self):
        ...
        # Barre de progression
        self.date_progress = ttk.Progressbar(
            progress_frame, orient="horizontal", length=500, mode="determinate"
        )
    ...
    def _init_organizer_tab(self):
        ...
        self.organizer_progress = ttk.Progressbar(
            progress_frame, orient="horizontal", length=500, mode="determinate"
        )
    ...
    def _init_exporter_tab(self):
        ...
        ttk.Radiobutton(
            mode_frame,
            text="📂 Export depuis structure organisée (par années)",
            variable=self.exporter_mode,
            value="organized",
            command=self._on_exporter_mode_change,
        ).pack(anchor="w", pady=5)

        ttk.Radiobutton(
            mode_frame,
            text="📄 Export direct depuis fichier URLs",
            variable=self.exporter_mode,
            value="direct",
            command=self._on_exporter_mode_change,
        ).pack(anchor="w", pady=5)
        ...
        self.organized_params_card = Card(
            left_frame, title="Paramètres d'Exportation Organisée"
        )
        ...
        self.direct_params_card = Card(
            left_frame, title="Paramètres d'Exportation Directe"
        )
        ...
        ttk.Label(
            self.years_scrollable.scrollable_frame,
        ...
        self.exporter_progress = ttk.Progressbar(
            progress_frame, orient="horizontal", length=500, mode="determinate"
        )
    ...
    def _init_keywords_tab(self):
        ...
        # Barre de progression
        self.keywords_progress = ttk.Progressbar(
            progress_frame, orient="horizontal", length=500, mode="determinate"
        )
    ...
    def _start_direct_exporter(self):
        ...
        self.current_process = threading.Thread(
            target=self._run_direct_exporter, args=(input_file, output_file)
        )
        self.current_process.daemon = True
        self.current_process.start()
        ...
        try:
            ...
            if result:
                ...
                self.queue.put(
                    (
                        "exporter_error",
                        "L'exportation directe n'a pas pu être terminée correctement.",
                    )
                )
    ...
    def _start_organized_exporter(self):
        ...
        if not base_dir:
            messagebox.showerror(
                "Erreur", "Veuillez sélectionner un répertoire de base."
            )
            return
        ...
        if not hasattr(self, "year_vars") or not self.year_vars:
            messagebox.showerror(
                "Erreur",
                "Aucune année disponible. Cliquez sur 'Scanner' pour détecter les années.",
            )
            return
        ...
        if combine:
            ...
            self.add_log_info(
                self.exporter_log, f"Années sélectionnées: {', '.join(selected_years)}"
            )
            if output_file:
                self.add_log_info(
                    self.exporter_log, f"Fichier de sortie: {output_file}"
                )
            else:
                default_name = f"urls_combinees_{'-'.join(selected_years)}.csv"
                self.add_log_info(
                    self.exporter_log, f"Fichier de sortie (par défaut): {default_name}"
                )
        else:
    ...
    def _run_keywords_search(self, input_file, output_file, keywords, max_threads):
        ...
        try:
            ...
            searcher = KeywordSearcher(
                input_file, keywords_list, case_sensitive, max_threads=max_threads
            )

            # Callback pour mettre à jour la progression
            def update_progress(current, max_val, message):
                eta, elapsed = self._calculate_eta(
                    current, max_val, self.keywords_start_time
                )
                eta_formatted = (
                    self.format_time(eta)
                    if eta != float("inf")
                ...
                enhanced_message = f"{message} | Temps écoulé: {elapsed_formatted} | ETA: {eta_formatted}"
                self.queue.put(
                    ("keywords_progress", current, max_val, enhanced_message)
                )
            ...
            if result:
                self.queue.put(
                    (
                        "keywords_complete",
                        result["stats"]["urls_with_matches"],
    ...
    def _update_keywords_progress(self, current, max_val, status):
        ...
        if hasattr(self, "keywords_stats_speed"):
            ...
            if max_val > 0 and current > 0:
                ...
                self.keywords_stats_progress.set(
                    f"{current}/{max_val} ({percent:.1f}%)"
                )
            else:
    ...
    def _keywords_complete(self, keywords_found, total_urls, output_file):
        ...
        self.add_log_separator(self.keywords_log)
        self.add_log_success(
            self.keywords_log, "Recherche de mots clés terminée avec succès!"
        )
        self.add_log_info(
            self.keywords_log, f"URLs avec mots clés: {keywords_found}/{total_urls}"
        )
        self.add_log_info(
            self.keywords_log, f"Les résultats ont été enregistrés dans {output_file}"
        )
        ...
        # Mettre à jour la barre de statut
        self.status_bar.set_status(
            f"Recherche terminée. {keywords_found} URLs contiennent les mots clés."
        )
    ...
    def _browse_organizer_output(self):
        ...
        directory = filedialog.askdirectory(
            initialdir=".", title="Sélectionner le répertoire de sortie"
        )
        if directory:
    ...
    def _scan_available_years(self):
        ...
        if not available_years:
            ...
            self.status_bar.set_status(
                "Aucune année trouvée dans le répertoire spécifié."
            )
            return

        # Boutons pour sélectionner toutes/aucune
        buttons_frame = ttk.Frame(
            self.years_scrollable.scrollable_frame, style="TFrame"
        )
        ...
        # Créer un séparateur
        ttk.Separator(self.years_scrollable.scrollable_frame, orient="horizontal").pack(
        ...
        for year in available_years:
            year_card = ttk.Frame(
                self.years_scrollable.scrollable_frame, style="Card.TFrame", padding=5
            )
        ...
        self.status_bar.set_status(
            f"{len(available_years)} années trouvées dans le répertoire spécifié."
        )
    ...
    def _select_all_years(self, select=True):
        ...
        status = (
            "Toutes les années sélectionnées."
            if select
            else "Toutes les années désélectionnées."
        )
    ...
    def _start_scraper(self):
        ...
        self.current_process = threading.Thread(
            target=self._run_scraper, args=(url, output_file, max_urls, self.stop_event)
        )
        self.current_process.daemon = True
        self.current_process.start()
    ...
    def _run_scraper(self, url, output_file, max_urls, stop_event):
        ...
        try:
            ...
            def update_progress(current, max_val, message):
                ...
                eta, elapsed = self._calculate_eta(
                    current, max_val, self.scraper_start_time
                )
                eta_formatted = (
                    self.format_time(eta)
                    if eta != float("inf")
            ...
            if stop_event.is_set():
                self.queue.put(
                    ("scraper_interrupted", len(scraper.found_urls), output_file)
                )
            else:
                # Mettre à jour l'interface avec le résultat
    ...
    def _scraper_interrupted(self, url_count, output_file):
        ...
        self.add_log_info(
            self.scraper_log, f"Nombre d'URLs trouvées avant interruption: {url_count}"
        )
        self.add_log_info(
            self.scraper_log, f"Les URLs ont été enregistrées dans {output_file}"
        )
    ...
    def _start_date_extractor(self):
        ...
        # Mettre à jour la barre de statut
        self.status_bar.set_status(
            f"Extraction des dates en cours pour {input_file}..."
        )

        # Démarrer l'extraction dans un thread séparé
        self.current_process = threading.Thread(
            target=self._run_date_extractor, args=(input_file, output_file, max_threads)
        )
        self.current_process.daemon = True
        self.current_process.start()
    ...
    def _run_date_extractor(self, input_file, output_file, max_threads):
        ...
        try:
            ...
            def update_progress(current, max_val, message):
                eta, elapsed = self._calculate_eta(
                    current, max_val, self.date_start_time
                )
                eta_formatted = (
                    self.format_time(eta)
                    if eta != float("inf")
    ...
    def _start_organizer(self):
        ...
        if not input_file:
            messagebox.showerror(
                "Erreur", "Veuillez sélectionner un fichier CSV d'entrée."
            )
            return
        ...
        # Mettre à jour la barre de statut
        self.status_bar.set_status(
            f"Organisation des URLs en cours pour {input_file}..."
        )

        # Démarrer l'organisation dans un thread séparé
        self.current_process = threading.Thread(
            target=self._run_organizer, args=(input_file, output_dir, folder_name)
        )
        self.current_process.daemon = True
        self.current_process.start()
    ...
    def add_log_header(self, log_widget, text):
        ...
        log_widget.tag_configure(
            "header", foreground=COLORS["primary_dark"], font=("Consolas", 11, "bold")
        )
    ...
    def add_log_info(self, log_widget, text):
        ...
        log_widget.tag_configure(
            "info", foreground=COLORS["text"], font=("Consolas", 9)
        )
        log_widget.insert(tk.END, f"• {text}\n", "info")
    ...
    def add_log_success(self, log_widget, text):
        ...
        log_widget.tag_configure(
            "success", foreground=COLORS["success"], font=("Consolas", 9, "bold")
        )
        log_widget.insert(tk.END, f"✓ {text}\n", "success")
    ...
    def add_log_error(self, log_widget, text):
        ...
        log_widget.tag_configure(
            "error", foreground=COLORS["error"], font=("Consolas", 9, "bold")
        )
        log_widget.insert(tk.END, f"✘ {text}\n", "error")
    ...
    def add_log_warning(self, log_widget, text):
        ...
        log_widget.tag_configure(
            "warning", foreground=COLORS["warning"], font=("Consolas", 9)
        )
        log_widget.insert(tk.END, f"⚠ {text}\n", "warning")
    ...
    def add_log_separator(self, log_widget):
        ...
        log_widget.tag_configure(
            "separator", foreground=COLORS["divider"], font=("Consolas", 9)
        )
    ...
    def _check_queue(self):
        ...
        try:
            for _ in range(
                10
            ):  # Traiter jusqu'à 10 messages à la fois pour une mise à jour plus fluide
    ...
    def _update_scraper_progress(self, current, max_val, status):
        ...
        if max_val > 0:
            self.status_bar.set_status(
                f"Scraping en cours: {current}/{max_val} URLs traitées"
            )
        else:
    ...
    def _scraper_complete(self, url_count, output_file):
        ...
        self.add_log_info(
            self.scraper_log, f"Nombre total d'URLs trouvées: {url_count}"
        )
        self.add_log_info(
            self.scraper_log, f"Les URLs ont été enregistrées dans {output_file}"
        )
    ...
    def _update_date_progress(self, current, max_val, status):
        ...
        if max_val > 0:
            self.status_bar.set_status(
                f"Extraction en cours: {current}/{max_val} URLs traitées"
            )
        else:
    ...
    def _date_error(self, error):
        ...
        messagebox.showerror(
            "Erreur d'extraction", f"Une erreur est survenue:\n{error}"
        )
    ...
    def _update_organizer_progress(self, current, max_val, status):
        ...
        if max_val > 0:
            self.status_bar.set_status(
                f"Organisation en cours: {current}/{max_val} URLs traitées"
            )
        else:
            self.status_bar.set_status(
                f"Organisation en cours: {current} URLs traitées"
            )
    ...
    def _organizer_complete(self, stats):
        ...
        if stats:
            ...
            self.add_log_info(
                self.organizer_log, f"URLs sans date: {urls_without_dates}"
            )
            ...
            if years:
                ...
                for year in sorted(years.keys()):
                    count = years[year]
                    percentage = (
                        (count / urls_with_dates) * 100 if urls_with_dates > 0 else 0
                    )
                    self.add_log_info(
                        self.organizer_log,
                        f"  {year}: {count} URLs ({percentage:.1f}%)",
                    )
        ...
        if messagebox.askyesno(
            "Continuer", "Voulez-vous passer à l'exportation CSV des URLs organisées?"
        ):
    ...
    def _organizer_error(self, error):
        ...
        messagebox.showerror(
            "Erreur d'organisation", f"Une erreur est survenue:\n{error}"
        )
    ...
    def _update_exporter_progress(self, current, max_val, status):
        ...
        if max_val > 0:
            self.status_bar.set_status(
                f"Exportation en cours: {current}/{max_val} étapes terminées"
            )
        else:
    ...
    def _exporter_complete(self, result, stats):
        ...
        if result:
            ...
            if export_type == "direct":
                ...
                self.add_log_info(
                    self.exporter_log, f"Total d'URLs exportées: {total_urls}"
                )
            ...
            self.add_log_warning(
                self.exporter_log, "L'exportation n'a pas pu être complétée."
            )
    ...
    def _exporter_error(self, error):
        ...
        messagebox.showerror(
            "Erreur d'exportation", f"Une erreur est survenue:\n{error}"
        )
    ...
    def _on_closing(self):
        ...
        if self.current_process and self.current_process.is_alive():
            if messagebox.askyesno(
                "Quitter", "Un processus est en cours. Voulez-vous vraiment quitter?"
            ):
    ...
    def _date_complete(self, dates_found, total_urls, output_file):
        ...
        self.add_log_separator(self.date_log)
        self.add_log_success(
            self.date_log, "Extraction des dates terminée avec succès!"
        )
        self.add_log_info(
            self.date_log, f"URLs avec dates trouvées: {dates_found}/{total_urls}"
        )
        self.add_log_info(
            self.date_log, f"Les résultats ont été enregistrés dans {output_file}"
        )
        ...
        # Mettre à jour la barre de statut
        self.status_bar.set_status(
            f"Extraction terminée. {dates_found}/{total_urls} URLs avec dates."
        )

        # Ajouter une pop-up de confirmation
        messagebox.showinfo(
            "Tâche terminée",
            f"L'extraction des dates est terminée!\n\nDates trouvées: {dates_found}/{total_urls}\nLes résultats ont été enregistrés dans:\n{output_file}",
        )

        # Proposer de passer à l'organisation
        if messagebox.askyesno(
            "Continuer", "Voulez-vous passer à l'organisation des URLs par date?"
        ):
